...
touch /opt/wireguard_server/wgset.conf
wgset="/opt/wireguard_server/wgset.conf"
...
function verify_domainname () {
      ip_address=$(ping -c 1 -n 1 -4 $domain_name | grep -Eo --max-count=1 '([0-9]{1,3}\.){3}[0-9]{1,3}')
...
function slave_availability () {
  ...
  read -r slave_true
    ...
    elif [ "$slave_true" != "m" ] && [ "$slave_true" != "s" ]; then
    echo "Invalid input. Please enter y or n"
    slave_availability
    fi  }

  function monitoring () {
  echo -n "Do you want to install wireguard exporter for monitoring server (y/n): "
  read -r monitoring_conf
  
  if [ -z "$monitoring_conf" ]; then
    echo "Please enter y or n"
    monitoring
  elif [ "$monitoring_conf" != "m" ] && [ "$monitoring_conf" != "s" ]; then
    echo "Invalid input. Please enter m or s"
    monitoring
  fi  }
...
cat /etc/wireguard/scripts/unit.conf > /etc/systemd/system/wg-quick@.service
...

{
  ...
  echo "* * * * * root systemctl reload wg-quick@wg0.service"  } >> /etc/crontab
...
monitoring
if [ $monitoring_conf == "n" ]; then 
 echo "Installation Complete!"
else
 echo "Installation wireguard exporter"
 cd /opt/wireguard_server/
 chmod +x prometheus_wireguard_exporter
 {
  echo "[Unit]"
  echo "Description=Prometheus WireGuard Exporter https://github.com/MindFlavor/prometheus_wireguard_exporter"
  ...
  echo "Group=root"
  echo "Type=simple"
  echo "ExecStart=/opt/wg_exporter/prometheus_wireguard_exporter -n $wgconf -i $wgDEV"

  echo "[Install]"
  echo "WantedBy=multi-user.target"
 } > wg_exporter.service

 mv wg_exporter.service /etc/systemd/system/
 systemctl daemon-reload
 systemctl start wg_exporter.service && systemctl enable wg_exporter.service
 echo "Start the binary with -h to get the complete syntax. " 
 echo "Once started, the tool will listen on the specified port (or the default one, 9586, if not specified)"
 echo "and return a Prometheus valid response at the url /metrics. So to check if the tool is working properly simply "
 echo "browse the http://localhost:9586/metrics (or whichever port you choose)."
